...
Vacuum expectation values of various combinations of field operators can be treated with varous Green functions of the Klein-Gordon equation.

For the K-G field, the expectation value of the commutator of the field operator can be represented by \tbf{Schwinger function} \(G(x,x')\):
\begin{equation}
  iG(x,x')=\expval{\left[\hat{\phi}(x),\hat{\phi}(x')\right]}{0}
\end{equation}
The anticommutator of field operator is defined as:
\begin{definition}[Anticommutator]
  \[\left\{\hat{\phi}(x),\hat{\phi}(x')\right\}=\hat{\phi}(x)\hat{\phi}(x')+\hat{\phi}(x')\hat{\phi}(x)\]
\end{definition}
The expectation value of anticommutator can be represented by \tbf{Hadamard's elementary function} \(G^{(1)}(x,x')\):
\begin{equation}
  G^{(1)}(x,x')=\expval{\left\{\hat{\phi}(x),\hat{\phi}(x')\right\}}{0}
\end{equation}
...
\begin{cthm}[Wightman functions]
  \begin{align*}
    G^+(x,x')&=\expval{\hat{\phi}(x)\hat{\phi}(x')}{0}\\
    G^-(x,x')&=\expval{\hat{\phi}(x')\hat{\phi}(x)}{0}.
  \end{align*}
\end{cthm}
...
\begin{cthm}[Feynman propagator]
  \begin{align*}
    iG_{F}(x,x')&=\expval{T(\hat{\phi}(x)\hat{\phi}(x'))}{0}\\
    &=\theta(t-t')G^+(x,x')+\theta(t'-t)G^-(x,x')
  \end{align*}
\end{cthm}
...
\begin{cthm}[Retarded/Advanced Green functions]
  \begin{align*}
    G_{R}(x,x')&=-\theta(t-t')G(x,x')\\
    G_{A}(x,x')&=\theta(t-t')G(x,x')
  \end{align*}
\end{cthm}
The average of retarded and advanced Green functions is denoted as 
\begin{equation}
  \bar{G}(x,x')=\frac{1}{2}\left[G_{R}(x,x')+G_{A}(x,x')\right].
\end{equation}
Then the Feynman propagator can be written as 
\begin{equation}
  G_{F}(x,x')=-\left[\bar{G}(x,x')+\frac{1}{2}iG^{(1)}(x,x')\right].
\end{equation}
The following relation are hold for those Green functions:
\begin{equation}
  (\Box+m^2)
  \begin{bmatrix*}[l]
    G(x,x')\\
...
For example, in the case that \(t'>t\), the contour for \(G_{R}(x,x')\) is closed by a semicircle in the upper complex plane.
...
\begin{example}
  ...
  \begin{equation*}
    G_F(x,x')=\frac{-\pi i}{(4\pi i)^{\frac{n}{2}}}\left[\frac{2m^2}{-\sigma+i\epsilon}\right]^{\frac{(n-2)}{4}}
