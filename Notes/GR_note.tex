...
\[R_{\mu\nu} = c T_{\mu\nu}\]
But, this is still not right, recall the conservation of energy, \(\nabla^{\mu}T_{\mu\nu} = 0\), the Ricci tensor does not fit this requirement.
Now, we need a rank 2 tensor that is conserved and fit the requirement above. We do have one, the Einstein tensor:
\[\nabla^{\mu}G_{\mu\nu} = 0\]
...
\begin{remark}\leavevmode
  There are a lot of different conventions to define the sign of the spacetime interval, here we just use the popular one \((-,+,+,+)\).
\end{remark}
...
These set of theories involves both the metric tensor and a scalar field, 
