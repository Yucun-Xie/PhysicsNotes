...
In general, we can just replace the partial derivative with covariant derivative, and replace flat spactime metric by \(g_{\mu\nu}\),
...
This is the famous Einstein field equation. We can recover the constant by requiring equation back to Newtonian limit.\\
To revover Newtonian graivty, impose the following boundary conditions to the field equation:
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item Vacuum, which mean outside the star, implying vanish of energy-momentum tensor. This mean the solution is not valid in the interior of star.
...
\end{cthm}
From this line element, we can notice a few property of Schwarzschild metric.
\begin{enumerate}
...
We can calculate the timelike geodesic from the metric, which implies planetary motion in general relativity, lead to famous result of precession of perihelion of Mercury.
We also able to calculate deflection of light by calculate lightlike geodesic from metric, this lead to the effect of \textbf{gravitational lensing}.

\subsubsection{Interior Solution}

Recall the Schwarzschild solution only valid outside the spherical star, not the interior, so there is no singularity and horizon in our Earth.
The interior solution can be solved by approximate the star by energy-momentum tensor of perfect fluid \(T_{\mu\nu} = (\rho + p)U_\mu U_\nu + pg_{\mu\nu}\),
and match the exterior solution at boundary. The interior solution of spherical star read \[ds^{2} =
...
Where \(r_{g}\) is the radius of surface.
...
Kerr solution is the vacuum solution to the axissymmetry rotating star, by apply following conditions to Einstein equation.
\begin{enumerate}
