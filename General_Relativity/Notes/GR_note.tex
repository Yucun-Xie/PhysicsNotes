...
\begin{definition}[Manifolds]
  ...
  \end{enumerate}
  More formally, an n-dimensional manifold is a topological space that is locally \tbf{homeomorphic} to the n-dimensional Euclidean space.
...
\begin{definition}[Weyl tensor]
  \[C_{\rho\sigma\mu\nu} = R_{\rho\sigma\mu\nu}-\frac{2}{(n-2)}\left[g_{\rho\left[\mu\right.}R_{\nu\left.\right]\sigma}
  -g_{\sigma\left[\mu\right.}R_{\nu\left.\right]\rho}\right]+
  \frac{2}{(n-1)(n-2)}\left[g_{\rho\left[\mu\right.}g_{\nu\left.\right]\sigma}R\right]\]
\end{definition}
...
\begin{definition}[Deodesic deviation equation]
  \[A^{\mu}=\frac{D^2}{\id t^2}S^{\mu}=R_{\nu\rho\sigma}^{\mu}T^{\nu}T^{\rho}S^{\sigma}\]
\end{definition}
...
\begin{definition}[Weak equivalence principle]
  The weak equivalence principle is stated as follows:
  \begin{enumerate}
